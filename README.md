...
개인적으로 주 무기로 사용할 언어 하나와 보조 무기로 사용할 언어 하나 정도 익혀두는 것을 추천합니다.
자신이 어떤 분야에서 일을 하게 될지, 어떤 분야를 세부적으로 전공할지 미리 정할 수 있다면 그에 맞춰서 주 무기를 구성하는 게 베스트지만, 그렇지 않은 경우가 더 많을 것입니다.
그렇다면 한국에서 일할 것이라면 Java를 주 무기로 삼고, Python을 보조 무기로 삼는 것을 추천합니다.
Java는 한국에서 가장 많이 사용하는 언어이고, Python은 범용적으로 쓸 수 있기 때문입니다. (코딩 테스트에 유리한 것은 덤이지요.)
...
* [모던 자바 인 액션](http://www.yes24.com/Product/Goods/77125987) : 자바 8 이 후 새로운 문법들이 많이 도입된 모던 자바를 효율적으로 사용할 수 있도록 가이드해주는 전문서입니다.
...
* [Head First Design Patterns](http://www.yes24.com/Product/Goods/108192370) : 유명한 디자인 패턴 입문서입니다. 사실 개인적으로 Head First 시리즈를 선호하는 편은 아니지만, 디자인 패턴만큼은 패턴들을 이해하고 익히기 좋은 예제로 잘 구성되어 있어서 예외로 두고 있습니다. 아래 GoF 서적이 디자인 패턴의 바이블이긴 하지만, 내용이 딱딱하고 일부 이해하기 쉽지 않아서 디자인 패턴을 처음 공부할 시에는 해당 서적으로 공부하는 것을 추천합니다. 참고로 디자인 패턴을 적용할 일이 많은 java로 작성되어 있습니다.
...
* [스프링 부트와 AWS로 혼자 구현한 웹 서비스](http://www.yes24.com/Product/Goods/83849117) : 스프링 부트를 이용하여 간단한 웹 서비스를 구현하며 스프링 부트 프레임워크와 웹 개발을 쉽게 입문할 수 있도록 해주는 기본서입니다.
* [HTTP 완벽 가이드](http://www.yes24.com/Product/Goods/15381085) : HTTP 프로토콜을 상세하게 설명해주느 레퍼런스 서적입니다.

### Database

* [Real MySQL](http://www.yes24.com/Product/Goods/103415627) : 데이터베이스를 학습함에 있어서 DBMS와 SQL 학습은 필수불가결합니다. 해당 서적은 세계에서 가장 많이 쓰이는 RDBMS인 MySQL을 국내에서 가장 상세하게 설명한 책입니다. 다만 굉장히 상세하고 실전적이라 처음 데이터베이스를 공부하는 분에게는 조금 버거울 수 있습니다. 그런 분들은 [모두의 SQL](http://www.yes24.com/Product/Goods/64434562)같은 입문서를 한번 빠르게 보거나 인터넷에 있는 쿼리문 예제들을 실습해보며 쿼리문을 조금 익히고 해당 서적을 보는 것을 추천합니다. 2022년 기준으로 MySQL 8.0 에 대응되는 개정판이 나왔습니다.
...
* [CMU 15-721](https://15721.courses.cs.cmu.edu/spring2020/) : 데이터베이스와 관련된 최신 학문적 트렌드를 다루는 수업입니다. 실제 수업 진행년도 기준으로 10년 이내의 논문들을 대표적인 사례들로 가져와서 리뷰해주는 방식입니다.
...
* [클린 코드 + 클린 아키텍처](http://www.yes24.com/Product/Goods/77988880) : 주로 협업을 하는 프로젝트에서 어떻게 코드를 유지보수하기 좋게 작성할지 가이드해주는 서적입니다. 유지보수뿐만 아니라 대형 프로젝트에 참여할 때 주의해야할 점들을 일깨워줍니다.
* [만들면서 배우는 클린 아키텍처](http://www.yes24.com/Product/Goods/105138479) : 헥사고날 아키텍처를 적용하여 유지보수에 유리한 프로젝트를 구성하는 방법에 대해 가이드해주는 서적입니다.
...
아래에는 보안쪽 공부를 하는데 있어 도움이 될만한 자료나 사이트들을 몇 가지 소개해드리겠습니다. 사실 필자는 와우해커나 해커즈랩이 주력이던 때에 달고나 문서 등으로 공부했던지라 최신 트렌드를 잘 모르기 때문에 고수분들이 PR을 주시면 굉장히 감사드리겠습니다.
...
* [우아한 테크코스](https://www.woowacourse.io/) : 우아한 형제들에서 진행하는 개발자 양성 교육 과정(부트캠프)입니다. 약 10개월간 진행되며, 매월 50 만원 ~ 130 만원의 교육비(KDT 가능 여부, 근로 장학생 등에 따라 지원되는 금액이 다릅니다)를 제공합니다. 클린 코드, 테스트 코드, 객체 지향 등에 대해 깊이 있게 고민해 볼 수 있고, 자바/스프링 기반의 팀 프로젝트를 수행합니다. 그리고 단순히 기술적인 부분 뿐만 아니라 개발자로서 살아가는데 필요한 마인드셋, 소프트 스킬에 대한 교육도 이루어져 많은 성장을 할 수 있습니다. 우아한 테크코스 수료생만을 대상으로 우아한 형제들에서 채용이 열리고, 5기부터는 딜리버리 히어로즈(독일)에서도 채용이 열리기 때문에 해외 취업에 관심이 있으시다면 노려보셔도 좋을 것 같습니다.
...
#### 커뮤니티 이용법
* [질문하는 법](http://1st.gamecodi.com/board/zboard.php?id=GAMECODI_Advice&no=29) : '온라인 커뮤니티에서 질문할 때', 질문자는 효율적으로 빠르게 원하는 답변을 받고, 답변자도 쾌적하게 선뜻 답변을 줄 수 있는 방법에 대해 다루고 있습니다. 요약하면 1)상대방에 대한 예의 지키기, 2)상황 설명하기, 3)코드와 결과 보여주기, 4)노력 어필하기, 5)답변에 감사표현하기, 6)질문글을 계속 업데이트하기 입니다. 
* [IT에서 좋은 질문을 하는 법?](https://blog.naver.com/tpgpfkwkem0/222462644688) : DBA [이재환](https://blog.naver.com/tpgpfkwkem0)님의, '온라인과 오프라인 회사, 그리고 사이트에서 질문할 때' 참고 하면 좋은 글입니다. 요약하면 1)본인이 해볼 만큼 해보기, 2)질문을 명확히 하기, 3)스스로 고민한 부분 공유하기, 4)질문의 환경을 상세히 정리하기, 5)답변자를 존중하기 마지막으로, 6)TPO 고려하기 입니다. 위의 <질문하는 법>이 '문제 해결'에 초점을 둔 글이라면, <IT에서 좋은 질문을 하는 법?>은 질문자의 '성장'에 초점을 둔 글입니다. 질문자가 성장하기 위해서, 질문할 때에는 '모르는 부분에 대해 포장하지 않고', '질문 후에는 답변을 자기 것으로 만들 수 있도록 본인이 정리하는 시간을 가지는 것'에 대한 중요성을 강조하고 있습니다.
* [개발자를 위한 정보 검색 팁](https://boxnwhis.kr/2020/09/27/ir-for-developers.html) : 강규영 님이 작성한 개발자를 위한 정보 검색 팁에 관한 글입니다. 구체적인 검색 방법뿐만 아니라 검색을 잘하기 위해 갖춰야 할 습관을 형성하는데 필요한 연습을 알려줍니다. 좋은 검색 습관 3가지는 1) 문제에 당면해서 검색하기, 2) 답을 이미 알고 있더라도 혹시 더 나은 방법이 있을까 싶어서 검색하기, 3) 갑자기 궁금해져서 검색하기입니다.
